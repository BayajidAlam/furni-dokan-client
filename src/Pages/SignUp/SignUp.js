...
const SignUp = () => {
  ...
  const saveUser = (userData) => {
    fetch("https://furni-dokan.vercel.app/user", {
      method: "POST",
      headers: {
  ...
  return (
    <div className="bg-[#DBEBFA] flex flex-col justify-center items-center pt-6">
      ...
      >
          ...
          {...register("password", {
            ...
            pattern: {
              value:
                /(?=.*[A-Z])(?=.*[!@#$&*])(?=.*[0-9])(?=.*[a-z].*[a-z]).{8}/,
              message:
                "password must have special number and uppdercase and number",
            },
          })}
        ...
        <select
          className="p-3 rounded-lg p mb-6"
          name="role"
          {...register("role")}
        >
        ...
        <input
          className="hover:bg-[#000000] text-[#FAFAFA] bg-[#49C0B6]  rounded-lg mb-1 p-3"
          type="submit"
        />
        ...
        <p className="text-center">
          ...
          <Link to="/login" className="text-[#1400F5]">
            Signup
          </Link>
        </p>
