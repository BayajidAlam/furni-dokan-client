...
const AddAProduct = () => {
  ...
  const { data: categories = [] } = useQuery({
    queryKey: ["categorys"],
    queryFn: () =>
      fetch("https://furni-dokan.vercel.app/categorys", {
        headers: {
  ...
  const { data: profileUser } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        `https://furni-dokan.vercel.app/user?email=${user?.email}`
      );
    ...
    fetch(url, {
      ...
      .then((imgData) => {
        ...
        if (profileUser !== "seller") {
          // send-server-db
          fetch("https://furni-dokan.vercel.app/category", {
            method: "POSt",
            headers: {
              ...
              <div className="form-control">
                ...
                >
                  {categories.map((category, i) => (
                    <option key={i} value={category.catName}>
                      {category.catName}
                    </option>
                  ))}
                </select>
